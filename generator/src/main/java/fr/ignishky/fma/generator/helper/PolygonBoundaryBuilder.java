...
public final class PolygonBoundaryBuilder {

    private PolygonBoundaryBuilder() {
    }

    public static void addPolygons(GeometrySerializer serializer, List<RelationMember> members, MultiPolygon multiPolygon, Map<String,
            String> wayTags) {
        ...
        IntStream.range(0, multiPolygon.getNumGeometries()).forEach(i -> {
            ...
            IntStream.range(0, polygon.getNumInteriorRing())
                    .forEach(j -> addRelationMember(serializer, wayTags, polygon.getInteriorRingN(j), "inner").ifPresent(members::add));

            addRelationMember(serializer, wayTags, polygon.getExteriorRing(), "outer").ifPresent(members::add);
        });
    }

    private static Optional<RelationMember> addRelationMember(GeometrySerializer serializer, Map<String, String> wayTags, LineString geom,
                                                              String memberRole) {
